package com.example.zzht.jmpatrol.view;
...
import com.zzz.ecity.android.applibrary.model.PropertyChangeModel;
 ...
 * @author gaokai
 */
public class NotificationImageView extends ImageView implements PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent event) {
        ...
        if (oldObj instanceof PropertyChangeModel) {
            ...
            if (contain(newModel.key, (String) this.getTag(R.id.tag_key))) {
                ...
                // ImageView上次隐含的数量
                int hideOldCount = (Integer) getTag(R.id.tag_count);
                int resultCount;
                // 工单数量减少
                if (oldCount > newCount) {
                    int count = oldCount - newCount;
                    resultCount = hideOldCount - count;
                ...
                if (resultCount > 0) {
                    ...
                    setImageDrawable(getContext().getResources().getDrawable(R.drawable.tips_red_no_text));
                } else {
                    iconState = state.DOWN;
                    // 当消息为0时，恢复下拉图标
                    setImageDrawable(getContext().getResources().getDrawable(R.drawable.icon_navbar_drop_down));
                }
            }
        }
    }
