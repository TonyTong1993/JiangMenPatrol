...
public class ConditionRectCommandXtd extends AMapMenuCommand {
    ...
    public boolean execute(MapView mapView, IMapOperationContext fragment) {
        if (null != mapView) {
            ...
            setMapFragmentTitleView(R.string.map_condition_rectquery);

            ConditionQueryDialog dialog = new ConditionQueryDialog(fragment.getContext(), mapView, fragment, false);
            dialog.show();
        }
        return true;
    }
    ...
    private void setMapFragmentTitleView(int resId) {
        ...
        mTitleView.setOnQueryAddressListener(((MapMainTabFragment)fragment).getQueryClickListener(false,false));
