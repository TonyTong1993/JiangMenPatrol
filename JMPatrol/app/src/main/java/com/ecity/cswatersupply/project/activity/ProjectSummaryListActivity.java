...
public class ProjectSummaryListActivity extends ABasePullToRefreshActivity<JSONObject> {
    ...
    protected ArrayListAdapter<JSONObject> prepareListViewAdapter() {
        return new ProjectSummaryListAdapter(this, new ProjectSummaryView());
    }
    ...
    protected OnItemClickListener prepareOnItemClickListener() {
        return new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                String section = dataSource.get(position).optString("section");
                String[] dates;
                if (month < 1) {
                    dates = getDateRangeByMonth(year, Integer.valueOf(section));
                } else {
                    dates = getDateRangeByDay(year, month, Integer.valueOf(section));
                }
                Intent intent = new Intent(ProjectSummaryListActivity.this, GeneralProjectListActivity.class);
                intent.putExtra(GeneralProjectListActivity.INTENT_KEY_PROJECT_STATUS, String.valueOf(-1));
                intent.putExtra(GeneralProjectListActivity.INTENT_KEY_PROJECT_TYPE, String.valueOf(type));
                intent.putExtra(GeneralProjectListActivity.INTENT_KEY_PROJECT_START_TIME, dates[0]);
                intent.putExtra(GeneralProjectListActivity.INTENT_KEY_PROJECT_END_TIME, dates[1]);
    ...
    private void requestSummaryInfo() {
        ProjectService.getInstance().querySummaryInfo(type, year, month, startTime, endTime);
    }

    private void handleGetSummaryInfoResponse(ResponseEvent event) {
        JSONObject response = event.getData();
        JSONArray summaryInfo = response.optJSONArray("items");
    ...
    public void onEventMainThread(ResponseEvent event) {
        ...
        switch (event.getId()) {
            case ResponseEventStatus.PROJECT_GET_PROJECT_SUMMARY_COUNT:
                LoadingDialogUtil.dismiss();
