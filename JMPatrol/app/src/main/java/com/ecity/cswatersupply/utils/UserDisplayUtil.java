...
public class UserDisplayUtil {
    ...
    private static Bitmap getUserMarkerOffline(){
        if(null == userMarkerOffline){
            try {
                userMarkerOffline = BitmapFactory.decodeResource(HostApplication.getApplication().getResources(), R.drawable.patrol_off); 
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private static Bitmap getUserMarkerOnline(){
        if(null == userMarkerOnline){
            try {
                userMarkerOnline = BitmapFactory.decodeResource(HostApplication.getApplication().getResources(), R.drawable.patrol_on); 
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private static Bitmap getUserMarkerHighLight(){
        if(null == userMarkerHighlight){
            try {
                userMarkerHighlight = BitmapFactory.decodeResource(HostApplication.getApplication().getResources(), R.drawable.patrol_highlight); 
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
