 ...
 *
 */
public class MeasureLengthCommandXtd extends AMapMenuCommand {
    ...
    public boolean execute(MapView mapView, IMapOperationContext fragment) {
        ...
        if (mapView.isLoaded()) {
            MapMeasure mapMeasure = new MapMeasure(fragment, mapView, LayerTool.getGraphicsLayer(mapView), MapMeasure.MeasureMode.Length);
            mapMeasure.start();
        }
        return true;
    }
