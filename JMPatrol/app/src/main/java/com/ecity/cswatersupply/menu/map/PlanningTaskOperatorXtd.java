 ...
 *
 */
public class PlanningTaskOperatorXtd extends AMapViewOperator {
    ...
    private final static int PTMAPNAVIGATIONVIEW_SHOW = 1;
    ...
    // 高亮显示
    public PictureMarkerSymbol POINT_HIGHLIGHT_MARKER_SYMBOL;
    //多人任务  其他用户到位
    ...
    public GraphicsLayer annimationLayer;
    public GraphicFlash graphicFlash;
    ...
    //map集合对应的key值
    public String[] pointPlanTaskGidsMapKeys = new String[]{"commenPointGidMap","arrivedPointGidMap"};
    //点图例参数 end
    ...
    //当前task属于点任务还是线任务
    public boolean isPointTask = false;
    //是否加载过图例
    ...
    public void setMapviewOption(MapView mapView, IMapOperationContext mapActivity) {
        ...
        activity.getTitleView().setBtnStyle(BtnStyle.RIGHT_ACTION);
        ...
        LINE_COMMON_MARKER_SYMBOL = new SimpleLineSymbol(activity.getResources().getColor(R.color.red), 10, SimpleLineSymbol.STYLE.SOLID);
        LINE_ARRIVED_MARKER_SYMBOL = new SimpleLineSymbol(activity.getResources().getColor(R.color.purple), 6, SimpleLineSymbol.STYLE.SOLID);
        TRACK_LINE_SYMBOL = new SimpleLineSymbol(activity.getResources().getColor(R.color.track_line_color), 10, SimpleLineSymbol.STYLE.SOLID);
        ...
        POLYGON_OUTLINE_SYMBOL = new SimpleLineSymbol(activity.getResources().getColor(R.color.geo_outline_color), 2, SimpleLineSymbol.STYLE.DASHDOT);
    ...
    public void drawImageLayer() {
        ...
        try {
            if (intent.hasExtra("planning_tasks")) {
                ...
                isPointTask = plantask.getLinelen() <= 0;
    ...
    private Graphic getMarkerGarhic(String taskidAndGid, Polyline line, Map<String, Object> mapForPlanId) {
    ...
    private void ControlViewDisplay(int view_show) {
        switch (view_show) {
            case MMAPLEGENDVIEW_SHOW:
                ptMapNavigationView.setVisibility(View.GONE);
    ...
    OnSingleTapListener m_OnSingleTapListener = new OnSingleTapListener() {
        ...
        private void selectOneGraphic(float x, float y) {
            ...
            if (layer != null && layer.isInitialized() && layer.isVisible()) {
                ...
                LogUtil.i("lonlat", "{" + lonlat[0] + "," + lonlat[1] + "}");
//                LogUtil.i("point",  "{" + point.getX() + "," + point.getY() + "}");
                if (result != null) {
                    // 获得附加特别的属性
                    ...
                    if ((null == pointPartClicked) && (null == linePartClicked)) {
                        ...
                        ControlViewDisplay(PTMAPNAVIGATIONVIEW_SHOW);
                        ptMapNavigationView.setOnNavigationListener(activity.new GoOnClickListener());
                        ptMapNavigationView.setOnReportEventListener(new ReportEventClickListener());
                    }
                }
            }
        }
        private void handleClickLine(Z3PlanTaskLinePart linePartClicked) {
            ...
            String LineId = linePartClicked.getGid() + "";
            //到位之后才能反馈 反馈的必定已经到位
            if (isArrived) {
                ptMapNavigationView.setDeviceTxtVisibility(View.VISIBLE);
                ptMapNavigationView.setAddressTxtVisibility(View.GONE);
                ptMapNavigationView.setDetailAddressTxtVisibility(View.GONE);
                ptMapNavigationView.setDeviceText(LineId + "(" + activity.getResources().getString(R.string.planningtask_arrived) + ")");
                ptMapNavigationView.setTextBackground(activity.getResources().getDrawable(R.drawable.textview_style_enable_true));
            } else {
                ptMapNavigationView.setDeviceTxtVisibility(View.VISIBLE);
                ptMapNavigationView.setAddressTxtVisibility(View.GONE);
                ptMapNavigationView.setDetailAddressTxtVisibility(View.GONE);
                ptMapNavigationView.setDeviceText(LineId + "(" + activity.getResources().getString(R.string.planningtask_not_arrived) + ")");
                ptMapNavigationView.setTextBackground(activity.getResources().getDrawable(R.drawable.textview_style_enable_false));
            }
        }
        ...
        private void renderMethod(GraphicsLayer gLayer, Graphic result) {
            ...
            Graphic g_pointNew = getHighlightGraphic(result);

            if (null != graphicFlash) {
                graphicFlash.stopFlash();
                graphicFlash = null;
            }
            graphicFlash = new GraphicFlash(result, g_pointNew, annimationLayer);
...
//    }
    ...
    private void getpPointInfoDetails() {
        if (pointPartClicked.getType().equals("0")) {
            handleClickPoint(pointPartClicked, "");
        }else {
    ...
    private void getpPointInfoDetailsByLocation(String infoDetails){
        ...
        if (isshow_infodetails) {
            ...
            if (null != infoList && infoList.size() > 0) {
                ...
                handleClickPoint(pointPartClicked, tips.toString());
            }
        }
    }

    private void handleClickPoint(Z3PlanTaskPointPart pointPartClicked, String tip) {
        //设置导航的目的地的经纬度
...
//        String pointTpyeAndId = null;
//        if ((pointPartClicked.getType().equals("0")) || (pointPartClicked.getEquipid().equals(null))) {
...
//        }
        ...
        if (isArrived) {
            ...
            ptMapNavigationView.setAddressTxtVisibility(View.GONE);
            ptMapNavigationView.setDetailAddressTxtVisibility(View.GONE);
            ptMapNavigationView.setDeviceText(tip + "(" + activity.getResources().getString(R.string.planningtask_arrived) + ")");
            ptMapNavigationView.setTextBackground(activity.getResources().getDrawable(R.drawable.textview_style_enable_true));
        } else {
            ptMapNavigationView.setDeviceTxtVisibility(View.VISIBLE);
            ptMapNavigationView.setAddressTxtVisibility(View.GONE);
            ptMapNavigationView.setDetailAddressTxtVisibility(View.GONE);
            ptMapNavigationView.setDeviceText(tip + "(" + activity.getResources().getString(R.string.planningtask_not_arrived) + ")");
            ptMapNavigationView.setTextBackground(activity.getResources().getDrawable(R.drawable.textview_style_enable_false));
        }
        ptMapNavigationView.setDetailListener(new OnDetailsClickListener());
    ...
    private void handleGetPointPartAttrs(ResponseEvent event) {
        ...
        LogUtil.i("ResponseEvent", eventObject);
    ...
    public void notifyBackEvent(IMapOperationContext mapActivity) {
        if (null != mapActivity) {
            ...
            if (null != commenPointGidMap) {
                commenPointGidMap.clear();
            }
            if (null != arrivedPointGidMap) {
                arrivedPointGidMap.clear();
            }
            mapActivity.finish();
    ...
    public void onEventMainThread(UIEvent event) {
        switch (event.getId()) {
            case UIEventStatus.PLANNINGTASK_TRACKLINE_NOTIFICATION:
                ...
                break;
            case UIEventStatus.PLANNINGTASK_REFLASH_XJ_POINT_STATUS:
