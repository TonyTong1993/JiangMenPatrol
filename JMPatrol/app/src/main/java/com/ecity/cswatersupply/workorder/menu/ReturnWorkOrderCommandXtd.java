...
public class ReturnWorkOrderCommandXtd extends AMenuCommand {
    ...
    public boolean executeWithExtra(View v, Bundle data) {
        ...
        data.putInt(CustomViewInflater.REPORT_TITLE, R.string.title_workorder_return);
        ...
        data.putString(WorkOrder.KEY_SUB_STATE, WorkOrderState.RETURN.value);
        data.putString(WorkOrderOperator.KEY_CACHE, ReturnWorkOrderCommandXtd.class.getName());
        data.putInt(WorkOrderOperator.KEY_EVENTID, UIEventStatus.WORKORDER_COMMON_INSPECT_REPORT);
        data.putInt(WorkOrderOperator.KEY_REPORT_SUCCESS_MSG, R.string.return_workorder_status);
