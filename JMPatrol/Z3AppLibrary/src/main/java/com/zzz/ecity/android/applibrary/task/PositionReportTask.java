...
public class PositionReportTask {
	...
	public boolean initReportPositionBeanBuilder(String className) {
		...
		try {
			reportPositionBeanBuilder = (AReportPositionBeanBuilder) Class
					.forName(className).newInstance();
		} catch (Exception e) {
			sendPositionBroadcast(EPositionMessageType.BUILDPOSITIONBEANERROR,
					e.getMessage());
		}
	...
	public synchronized void addFilterLocation(Location location) {
		...
		if (PositionConfig.POINTS_PER_MINUTE > 60
				|| PositionConfig.POINTS_PER_MINUTE < 0) {
			...
			if (null != xys && xys.size() > 0) {
				try {
					if (null == lastReportLocation) {
						...
						long timeOffset = 0;
						timeOffset = calculateTimeOffsetSecond(
					...
					sendPositionBroadcast(
							EPositionMessageType.BUILDPOSITIONBEANERROR,
							e.getMessage());
				}
			}
		...
		if (xys != null && xys.size() > 0) {
			...
			try {
				if (1 == currentFilterData.length) {
					if (!isSameLocation(lastReportLocation,
							currentFilterData[0])) {
						try {
					...
					for (int i = 0; i < currentFilterData.length - 1; i++) {
						if (null == lastReportLocation) {
							...
							tf = calculateTimeOffsetSecond(lastReportLocation,
									loca);
							if (tf < timeSpacingSecond) {
								double dt = MercatorUtil
										.calculateLength(lastReportLocation
												.getLongitude(),
												lastReportLocation
														.getLatitude(), loca
														.getLongitude(), loca
	...
	private boolean isSameLocation(Location a, Location b) {
		...
		if (Math.abs(a.getLongitude() - b.getLongitude()) < 0.000001
				&& Math.abs(a.getLatitude() - b.getLatitude()) < 0.000001
	...
	private void buildReportBean(Location location) {
		if (location == null || MyApplication.getApplication() == null
				|| StringUtil.isBlank(PositionConfig.getReportUserId())) {
			return;
		}
		...
		if (null != reportPositionBeanBuilder) {
			...
			sendPositionBroadcast(
					EPositionMessageType.POSITIONBEANBUILDERNOTFOUND, "");
			if (null == defaultReportPositionBeanBuilder) {
				defaultReportPositionBeanBuilder = new DefaultReportPositionBeanBuilder();
			}

			bean = defaultReportPositionBeanBuilder
					.buildGPSPositionBean(location);
		}
		if (null != bean) {
			savePosition(bean);
		}
	...
	private void savePosition(GPSPositionBean bean) {
		...
		try {
			...
			e.printStackTrace();
			sendPositionBroadcast(EPositionMessageType.POSITIONBEANSAVEFAIL,
					e.getMessage());
		}
	}
