...
public class LoadingDialogUtil {
    ...
    public static final void show(Activity activity, int messageId,int id) {
        Activity mActivity = new WeakReference<Activity>(activity).get();
        String message = activity.getString(messageId);
        show(mActivity, message+id);
    }
    ...
    public static final void updateMessage(int messageId) {
        String message = MyApplication.getApplication().getString(messageId);
        updateMessage(message);
    }
