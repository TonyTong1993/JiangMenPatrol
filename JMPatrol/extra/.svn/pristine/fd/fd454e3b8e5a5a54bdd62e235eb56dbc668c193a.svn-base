...
public abstract class TableHeaderAdapter {
    private final TableHeaderConfig config;
    private final Context context;
    protected final TableHeaderColumnModel columnModel;

    protected TableHeaderAdapter(final Context context, final TableHeaderColumnModel columnModel,final TableHeaderConfig config) {
        this.context = context;
        this.columnModel = columnModel;
        if(null == config){
            this.config = new TableHeaderConfig();
        } else {
            this.config = config;
        }
    }
     ...
     * @return
     */
    public TableHeaderConfig getConfig() {
        return config;
    }
