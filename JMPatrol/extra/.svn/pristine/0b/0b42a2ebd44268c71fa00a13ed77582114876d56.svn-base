...
class TableHeaderView extends LinearLayout {
    ...
    public void setAdapter(final TableHeaderAdapter adapter) {
        ...
        LayoutParams laParams = (LayoutParams) getLayoutParams();

        if (!adapter.getConfig().showHeader) {
            laParams.width = ViewGroup.LayoutParams.MATCH_PARENT;
            laParams.height = 0;
        } else {
            laParams.width = ViewGroup.LayoutParams.MATCH_PARENT;
            laParams.height = ViewGroup.LayoutParams.WRAP_CONTENT;
        }
    ...
    protected void renderHeaderViews() {
        ...
        for (int columnIndex = 0; columnIndex < adapter.getColumnCount(); columnIndex++) {
            ...
            headerView.setBackgroundColor(adapter.getConfig().titleBackgroundColor);
