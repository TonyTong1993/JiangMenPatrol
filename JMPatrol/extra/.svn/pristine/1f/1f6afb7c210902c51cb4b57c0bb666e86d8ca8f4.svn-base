...
public class GeometryUtil {
    ...
    public static ArrayList<Point> getPoints(MultiPath multiPath) {
        ...
        try {
            if (multiPath instanceof Polyline) {
                for (int i = 0; i < ((Polyline) multiPath).getPointCount(); i++) {
                    points.add(((Polyline) multiPath).getPoint(i));
                }
            } else {
                for (int i = 0; i < ((Polygon) multiPath).getPointCount(); i++) {
                    points.add(((Polygon) multiPath).getPoint(i));
                }
                points.add(((Polygon) multiPath).getPoint(0));
            }
        } catch (Exception e) {
            points = null;
        }
    ...
    public static boolean IsEnvelopeContainPoint(Point point, Envelope envelope) {
        ...
        if (point.getX() > envelope.getXMin()
     ...
     * @return
     */
    public static boolean IsCircleContainPoint(Point point, Point circleCenter,
                                               double r) {
    ...
    public static Geometry transformLonLat2Mercator(Geometry geo) {
        ...
        if (geo instanceof Point) {
            ...
            for (int i = 0; i < src.getPointCount(); i++) {
                double[] xy;
                try {
                    xy = lonLat2Mercator(src.getPoint(i).getX(), src
                            .getPoint(i).getY());
